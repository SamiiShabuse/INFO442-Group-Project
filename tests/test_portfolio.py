...
def test_build_rf_predicted_covariance_matrix_chooses_date_before_missing_asset_fallback():
    assets = ["AAPL", "MSFT", "GLD"]
    prediction_date = pd.Timestamp("2026-01-05")
    rf_vol = pd.DataFrame(
        {
        ...
        index=[prediction_date, pd.Timestamp("2026-01-06")],
    )
    correlation = pd.DataFrame(np.eye(3), index=assets, columns=assets)
    historical_volatility = pd.Series({"AAPL": 0.11, "MSFT": 0.21, "GLD": 0.30})
...
def test_optimize_portfolio_rejects_infeasible_weight_cap():
    assets = ["AAPL", "MSFT"]
    mean_returns = pd.Series([0.001, 0.0012], index=assets)
    covariance = pd.DataFrame(np.eye(2) * 0.0001, index=assets, columns=assets)
