"""Helpers for turning target portfolio weights into dry-run rebalance orders."""
...
def load_current_positions(path: str | None) -> pd.DataFrame:
    ...
    positions["quantity"] = pd.to_numeric(
...
def generate_rebalance_orders(
    ...
    current_positions = (
        current_positions if current_positions is not None else load_current_positions(None)
    )
    current_qty = (
        current_positions.set_index("ticker")["quantity"]
